...
/**
 * Harry's Farm - GSheet Realtime V133
 *
 * Cara pasang:
...
function doPost(e) {
  ...
  try {
    ...
    writeKeluarMasukRealtime_(ss, data, date);
    writeBarangDipakaiRealtime_(ss, data, date);
    writePemakaianProduk_(ss, data, date);

    log.appendRow([new Date(), 'EXPORT', 'OK', 'Realtime keluar/masuk + barang dipakai + stok masuk ke Google Sheet', date, ((data.keluar_masuk || data.transaksi || []).length), (data.stok || []).length]);
...
function readPayload_(e) {
  ...
  if (raw.indexOf('hf_action=') >= 0) {
    throw new Error('Payload JSON tidak terbaca. Apps Script menerima form tanpa field payload. Pakai file GOOGLE_APPS_SCRIPT_V133_REALTIME_KELUAR_MASUK_PEMAKAIAN.gs dan deploy New version.');
  }
...
function writeDailyReport_(ss, data, date) {
  ...
  pushTitle_(rows, 'BARANG DIPAKAI - BAHAN KELUAR PRODUKSI');
  rows.push(['Kode','Tanggal','Jam','Arah','Jenis','Nama Barang','Kategori','Keluar','Masuk','Stok Saat Ini','Satuan','Jenis Dus','Jenis Plastik','Keterangan','Petugas']);
  (data.barang_dipakai || []).forEach(function(x){
    rows.push([v_(x.kode),v_(x.tanggal),v_(x.jam),v_(x.arah),v_(x.jenis),v_(x.nama_barang),v_(x.kategori),n_(x.keluar_pcs),n_(x.masuk_pcs),v_(x.stok_saat_ini),v_(x.satuan),v_(x.jenis_dus),v_(x.jenis_plastik),v_(x.keterangan),v_(x.petugas)]);
  });
  if(!(data.barang_dipakai || []).length) rows.push(['Belum ada barang dipakai hari ini']);
  rows.push([]);

  pushTitle_(rows, 'PEMAKAIAN BAHAN PER PRODUK');
  rows.push(['Kode Produksi','Tanggal','Jam','Produk','Hasil Masuk','Satuan Produk','Bahan Dipakai','Kategori Bahan','Jumlah Keluar','Satuan Bahan','Petugas','Catatan']);
  (data.pemakaian_produk || []).forEach(function(x){
    rows.push([v_(x.kode_produksi),v_(x.tanggal),v_(x.jam),v_(x.produk),n_(x.hasil_masuk),v_(x.satuan_produk),v_(x.bahan_dipakai),v_(x.kategori_bahan),n_(x.jumlah_keluar),v_(x.satuan_bahan),v_(x.petugas),v_(x.catatan)]);
  });
  if(!(data.pemakaian_produk || []).length) rows.push(['Belum ada pemakaian produk hari ini']);
  rows.push([]);
...
function writeTransaksi_(ss, data, date) {
  ...
  var header = ['Waktu Export','Tanggal','Jam','Arah','Jenis','No SJ/DO','Tujuan','Nama Barang','Kategori','Keluar Pcs','Keluar Dus','Keluar Item','Masuk Pcs','Masuk Dus','Masuk Item','Stok Saat Ini','Satuan','Keterangan','Petugas'];
  ensureHeader_(sh, header);
  removeRowsByDate_(sh, date, 2);
  var list = data.keluar_masuk || data.transaksi || [];
  list.forEach(function(x){
    sh.appendRow([new Date(),v_(x.tanggal || date),v_(x.jam),v_(x.arah),v_(x.jenis),v_(x.no_surat_jalan),v_(x.tujuan),v_(x.nama_barang),v_(x.kategori),n_(x.keluar_pcs),n_(x.keluar_dus),n_(x.keluar_item),n_(x.masuk_pcs),n_(x.masuk_dus),n_(x.masuk_item),v_(x.stok_saat_ini),v_(x.satuan),v_(x.keterangan),v_(x.petugas)]);
  });
}

function writeKeluarMasukRealtime_(ss, data, date) {
  var sh = getOrCreateSheet_(ss, 'KELUAR_MASUK_REALTIME');
  sh.clear();
  var rows = [['Update', new Date(), 'Tanggal', date], []];
  rows.push(['Kode','Tanggal','Jam','Arah','Jenis','No SJ/DO','Tujuan','Nama Barang','Kategori','Area','Lokasi','QC','Kemasan','Keluar Pcs','Keluar Dus','Keluar Item','Masuk Pcs','Masuk Dus','Masuk Item','Stok Saat Ini','Satuan','Keterangan','Petugas']);
  (data.keluar_masuk || data.transaksi || []).forEach(function(x){
    rows.push([v_(x.kode),v_(x.tanggal),v_(x.jam),v_(x.arah),v_(x.jenis),v_(x.no_surat_jalan),v_(x.tujuan),v_(x.nama_barang),v_(x.kategori),v_(x.area),v_(x.lokasi),v_(x.qc),v_(x.kemasan),n_(x.keluar_pcs),n_(x.keluar_dus),n_(x.keluar_item),n_(x.masuk_pcs),n_(x.masuk_dus),n_(x.masuk_item),v_(x.stok_saat_ini),v_(x.satuan),v_(x.keterangan),v_(x.petugas)]);
  });
  writeRows_(sh, rows);
  sh.setFrozenRows(3);
}

function writeBarangDipakaiRealtime_(ss, data, date) {
  var sh = getOrCreateSheet_(ss, 'BARANG_DIPAKAI_REALTIME');
  sh.clear();
  var rows = [['Update', new Date(), 'Tanggal', date], []];
  rows.push(['Kode','Tanggal','Jam','Arah','Jenis','Nama Barang','Kategori','Area','Lokasi','Keluar','Masuk','Stok Saat Ini','Satuan','Jenis Dus','Jenis Plastik','Keterangan','Petugas']);
  (data.barang_dipakai || []).forEach(function(x){
    rows.push([v_(x.kode),v_(x.tanggal),v_(x.jam),v_(x.arah),v_(x.jenis),v_(x.nama_barang),v_(x.kategori),v_(x.area),v_(x.lokasi),n_(x.keluar_pcs),n_(x.masuk_pcs),v_(x.stok_saat_ini),v_(x.satuan),v_(x.jenis_dus),v_(x.jenis_plastik),v_(x.keterangan),v_(x.petugas)]);
  });
  if(!(data.barang_dipakai || []).length) rows.push(['Belum ada barang dipakai hari ini']);
  writeRows_(sh, rows);
  sh.setFrozenRows(3);
...
function writePemakaianProduk_(ss, data, date) {
  var sh = getOrCreateSheet_(ss, 'PEMAKAIAN_PRODUK');
  sh.clear();
  var rows = [['Update', new Date(), 'Tanggal', date], []];
  rows.push(['Kode Produksi','Tanggal','Jam','Produk','Hasil Masuk','Satuan Produk','Bahan Dipakai','Kategori Bahan','Jumlah Keluar','Satuan Bahan','Petugas','Catatan']);
  (data.pemakaian_produk || []).forEach(function(x){
...
function ensureHeader_(sh, header) {
  ...
  var current = sh.getLastRow() ? sh.getRange(1, 1, 1, Math.max(1, lastCol)).getValues()[0].join('|') : '';
  if (sh.getLastRow() === 0 || current.indexOf(header[0]) !== 0 || current.indexOf('Arah') < 0) {
    sh.clear();
    sh.appendRow(header);
    sh.setFrozenRows(1);
  }
}
function removeRowsByDate_(sh, date, dateCol) {
  var last = sh.getLastRow();
  if (last < 2) return;
  var values = sh.getRange(2, dateCol, last - 1, 1).getValues();
  for (var i = values.length - 1; i >= 0; i--) {
    if (String(values[i][0]) === String(date)) sh.deleteRow(i + 2);
  }
}
...
function styleSheet_(sh, rowCount, colCount) {
  ...
  for (var r = 1; r <= rowCount; r++) {
    ...
    if (String(v).match(/^(RINGKASAN|TRANSAKSI HARIAN|BARANG DIPAKAI - BAHAN KELUAR PRODUKSI|PEMAKAIAN BAHAN PER PRODUK|STOK KRITIS|SNAPSHOT SEMUA STOK)$/)) {
