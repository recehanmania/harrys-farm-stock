CARA PAKAI V3 - HARRY'S FARM STOCK ONLINE

Versi V3 ini tidak memakai CDN/library luar, jadi lebih aman dari blank screen.

1. Extract ZIP.
2. Buka file config.js.
3. Isi publishable key dari Supabase:
   anonKey: "sb_publishable_xxxxx"
4. Save.
5. Upload SEMUA isi folder harrys_farm_stock_online_v3 ke Netlify.

PENTING:
- Upload folder yang di dalamnya langsung ada index.html, app.js, styles.css, config.js.
- Jangan upload folder yang salah.
- Jangan pakai SECRET KEY. Pakai Publishable key.
- Kalau aplikasi error, V3 akan menampilkan tulisan error, bukan blank.

DATABASE:
Kalau SQL sebelumnya sudah berhasil dengan hasil setval 45, tidak perlu jalankan SQL lagi.
Kalau belum, jalankan supabase-schema.sql di Supabase SQL Editor.


FIX V4:
- Memperbaiki error "No API key found in request" saat klik Simpan.
- Penyebabnya header apikey tertimpa saat request POST/PATCH/DELETE.
- Upload ulang folder V4 ini ke Netlify.
