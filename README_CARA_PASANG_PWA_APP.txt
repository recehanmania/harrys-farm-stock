HARRY'S FARM STOCK OPNAME - V6 PWA APP

Versi ini sudah dibuat seperti aplikasi HP/PC:
- Ada manifest.json
- Ada service-worker.js
- Ada icon aplikasi
- Bisa Add to Home Screen / Install App dari Chrome
- Tetap pakai login staff dari versi V5

CARA PASANG:
1. Extract ZIP.
2. Buka config.js.
3. Isi Publishable key Supabase.
4. Upload folder harrys_farm_stock_app_pwa_v6 ke Netlify.
5. Buka link Netlify di HP.
6. Di Chrome klik titik tiga > Tambahkan ke layar utama / Install app.
7. Aplikasi akan muncul di layar HP dengan nama HF Stock.

PENTING:
- Kalau Supabase RLS staff login sudah dijalankan sebelumnya, tidak perlu jalankan SQL lagi.
- Kalau belum, jalankan supabase-secure-staff-login.sql di Supabase SQL Editor.
- Jangan pakai Secret key. Pakai Publishable key saja.


UPDATE V7 LOGO:
- Logo aplikasi sudah diganti memakai logo Harry's Farm yang diupload.
- Icon PWA, favicon, logo login, dan logo header sudah diganti.
- Setelah upload ulang ke Netlify, kalau icon lama masih muncul, hapus cache browser atau install ulang PWA dari HP.


UPDATE V8 GRAFIK + TABEL:
- Dashboard ditambah grafik status stok.
- Dashboard ditambah grafik transaksi 7 hari.
- Ditambah tabel ringkasan kategori.
- Ditambah tabel stok paling kritis.
- Tetap memakai login staff dan Supabase.


UPDATE V9 CACHE FIX:
- Cache PWA dinaikkan ke versi V9 supaya grafik/tabel baru muncul.
- Setelah upload ke Netlify, uninstall aplikasi lama di HP lalu install ulang.
- Kalau masih belum berubah, buka Chrome > Settings > Site settings > All sites > cari harrys-farm-stock.netlify.app > Clear data.
